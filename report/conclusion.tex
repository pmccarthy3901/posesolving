...
The basin sweeps showed that convergence degrades as the initialisation error increases, and that failure is discrete. The LM solver either converges or fails outright, and on a cold start it fails on $14\%$ of test poses. The network alone is reliable but imprecise. Its errors are unimodal and almost never reach the desired convergence tolerances. The two solvers are therefore complementary. The hybrid solver derived by combining the two retains the precision of the LM solver, but allows the NN initialisation to provide increased reliability. On the held-out test set, the hybrid raises the convergence rate from $0.859$ to $0.947$, reduces the 95th percentile positional error from $64.2$mm to $1.67$mm, and runs $2.8$ times faster than a cold start LM solver.
