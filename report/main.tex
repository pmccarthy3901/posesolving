\documentclass[12pt,a4paper]{report}

\setcounter{secnumdepth}{3} %The level of sections that get numbered
\setcounter{tocdepth}{3} %The level of sections to appear in ToC
\usepackage{url}
\usepackage{graphicx}
\usepackage{float}
\usepackage{subcaption}
\usepackage{braket}
\usepackage{makecell}
\usepackage{multirow}
\usepackage{calligra}
\usepackage{listings}
\usepackage{hyperref}
\usepackage{amssymb}
\usepackage{amsmath}
\usepackage{xcolor}
\usepackage{aurical}
\usepackage[T1]{fontenc}
\usepackage{booktabs}
\usepackage{svg}
%Date formatting Package
\usepackage[ddmmyyyy]{datetime}

\usepackage{wrapfig}

\definecolor{codegreen}{rgb}{0.133,0.545,0.133}
\definecolor{codegray}{rgb}{0.5,0.5,0.5}
\definecolor{codepurple}{rgb}{0.58,0,0.827}
\definecolor{codeblue}{rgb}{0.0,0.39,0.75}
\definecolor{codeback}{rgb}{0.97,0.97,0.97}
\lstdefinestyle{python}{
  language=Python,
  backgroundcolor=\color{codeback},
  commentstyle=\color{codegreen},
  keywordstyle=\color{codeblue}\bfseries,
  stringstyle=\color{codepurple},
  numberstyle=\tiny\color{codegray},
  basicstyle=\ttfamily\small,
  breakatwhitespace=false,
  breaklines=true,
  keepspaces=true,
  numbers=left,
  numbersep=5pt,
  showspaces=false,
  showstringspaces=false,
  showtabs=false,
  tabsize=4,
  xleftmargin=0pt,
  xrightmargin=0pt,
  framesep=4pt,
  frame=single,
  rulecolor=\color{codegray},
}


\newtheorem{definition}{Definition}[section]

\begin{document}
\begin{titlepage} \vspace{1cm}

\begin{center}
\textbf{\Huge Pose Solving Algorithms for Electromagnetic Tracking}
\end{center}

\vspace{1.0cm}

\begin{figure}[h]
    \centering
    \includegraphics[scale = 0.7]{figs/UCC_logo} 
\end{figure}

\vspace{1.0cm}

\begin{center}
\textbf{\Large Author:} \Large{Patrick McCarthy} {\LARGE\par}
\textbf{\Large Supervisor:} \Large{Dr. Andreas Amann} {\LARGE\par}
\end{center}
\vspace{1.0cm}

\begin{center}
\Large
A thesis submitted in partial fulfilment of the requirements for the degree of\\ MSc Mathematical Modelling and Machine Learning
\vfill{}
\Large{School of Mathematical Sciences, \\
University College Cork, \\
Ireland \\
\vspace{0.4cm}
September 2026}
\end{center}

\end{titlepage}


\chapter*{Declaration of Authorship}

This report is wholly the work of the author, except where explicitly
stated otherwise. The source of any material which was not created
by the author has been clearly cited. \\
\medskip{}

\textbf{Date:} \quad \today

\medskip{}

\textbf{Signature:}  Patrick McCarthy

\chapter*{Acknowledgements}

I would like to thank my supervisor, Dr. Andreas Amann, for his guidance and feedback throughout the project, and for his patience in helping me shape both the direction of the work and how it is presented. 

I am grateful to Dr. Alex Jaeger for his generosity with his time, expertise, and access to hardware throughout this project. 

Finally, I would like to thank my friends and family for their continued support and encouragement, without whom this thesis would not have been possible. Particularly I would like to thank Seán and Laura who helped with proofreading.

\chapter*{Abstract}

Electromagnetic tracking systems recover sensor pose by solving a nonlinear inverse problem. This thesis simulates one such electromagnetic tracking system and characterises three solvers. The standard solver is Levenberg-Marquardt, which is precise but struggles with initialisation on cold start. Neural networks are less precise but do not have the initialisation issue. This thesis builds a hybrid solver which uses a neural network to initialise a Levenberg-Marquardt solver, allowing for better precision than neural networks alone, and better reliability than Levenberg-Marquardt alone. On a held-out test set the hybrid solver raises the convergence rate, reduces the error, and runs almost three times faster than a cold-start Levenberg-Marquardt solver. 

\tableofcontents{}

\newpage
\chapter*{List of Abbreviations}
\begin{description}
  \item[LM] Levenberg-Marquardt 
  \item[NN] Neural Network
  \item[EMT] Electromagnetic Tracking
  \item[PCB] Printed Circuit Board
  \item[GN] Gauss-Newton 
  \item[GD] Gradient Descent
  \item[CPU] Central Processing Unit 
  \item[GPU] Graphics Processing Unit 
  \item[MSE] Mean Squared Error
  \item[UCC] University College Cork
\end{description}
\listoffigures{}
\listoftables{}
\newpage 
\input{intro}

\newpage
\input{background}

\newpage
\input{methods}

\newpage
\input{results}

\newpage 
\input{conclusion}

\newpage
\bibliographystyle{unsrt}
\bibliography{references}

\newpage 
\appendix
\input{appendix}




\end{document}
